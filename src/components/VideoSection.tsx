...
const VideoSection = () => {
  ...
  return (
    <>
      ...
      <section className="py-10 md:py-16 relative overflow-hidden" style={{ backgroundColor: '#E5E5EA', paddingBottom: '6%' }}>
        ...
        <div className="container px-4 relative z-10">
          <div className="max-w-6xl mx-auto">
            {/* Heading */}
            <h2 className="h2-heading font-bold text-center mb-8 md:mb-12" style={{ color: '#043880' }}>
             Trusted by leading organizations for seamless hiring and onboarding 
            </h2>
            {/* Desktop: Grid Layout (hidden on mobile) */}
            <div className="hidden md:grid grid-cols-4 gap-6">
              {logos.map((logo, index) => (
                <div key={index} className="bg-white p-6 flex items-center justify-center">
                  <img
                    src={logo.src}
                    alt={logo.alt}
                ...
                <div className="animate-infinite-scroll flex gap-6">
                  ...
                  {logos.map((logo, index) => (
                    <div key={`first-${index}`} className="flex-shrink-0 w-[200px]">
                      <div className="bg-white p-6 flex items-center justify-center h-[120px]">
                        <img
                          src={logo.src}
                          alt={logo.alt}
                          className="w-full h-auto object-contain max-h-16"
